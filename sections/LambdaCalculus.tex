...
\begin{proof}
    ...
    If $t$ is not a $\lambda$-term then $(tu)^* = t^*u^*$, so we are done by Definition \ref{def:parared} (iii).
...
\begin{boxlem}\label{lem:computableSN}
    \hfill
    \begin{enumerate}
...
\begin{boxlem}\label{lem:lambdaconstruct}
