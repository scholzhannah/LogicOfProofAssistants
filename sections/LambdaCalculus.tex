...
\begin{boxdefi}
    ...
    The terms of the $\lambda$-calculus are given by the following BNF: 
    \begin{equation*}
...
\begin{boxdefi}
    We can define the \alert{pairing} $\alert{P} = \lambda stx.xst$ and denote $\alert{(s, t)} \coloneq Pst \bered \lambda x. xst$.
\end{boxdefi}
...
\begin{boxlem}
    If $t \Rightarrow t'$ and $w \Rightarrow w'$ then $t[w/x] \Rightarrow t'[w'/y]$.
\end{boxlem}
\begin{proof}
    We show this by induction on $t \Rightarrow t'$.
...
\begin{boxdefi}
    If $t$ is a term then $t^*$ is recursively defined as 
    \begin{enumerate}
...
\begin{proof}
    ...
    If the last step was Definition \ref{def:parared} (iii), i.e. concluding $tu \Rightarrow t'u'$ from $t \Rightarrow t'$ and $u \Rightarrow u'$.
    By induction hypothesis $t' \Rightarrow t^*$ and $u' \Rightarrow u^*$.
    ...
    We know $\lambda x.s \Rightarrow t'$ which could only be derived from Definition \ref{def:parared} (ii). 
    So $t' = \lambda x. s'$ with $s \Rightarrow s'$.
    By induction hypothesis $s' \Rightarrow s^*$. 
    Then $(\lambda x. s') u' \Rightarrow s^* [u*/x]$ follows from (iv).
