...
\begin{boxdefi}
    A $\lambda$-term $t$ \alert{respresents} $f \colon \bN^k \to \bN$ if for all $\vect{v} \in \bN^k$ we have $t[n_0] \dots [n_{k-1}] \bered [f(\vect{n})]$ if $f(\vect{n})\down$ and that $t[n_0] \dots [n_{k-1}]$ has no normal form if $f(\vect{n})\up$.
\end{boxdefi}
...
Then $f \circ \vect{g}$ is represented by $\lambda \vect{x}. F (G_0 \vect{x}) (G_1 \vect{x}) \dots (G_{k-1} \vect{x})$.
Lastly, for (v) suppose that $F_0$ represents $f_0$ and $F_s$ represents $f_s$. 
We want to find $G$ such that 
\begin{equation*}
    G[0]\vect{x} \beq F_0 \vect{x} \text{\quad and \quad} G[n + 1]\vect{x} \beq F_s [n] (G[n] \vect{x})\vect{x}
\end{equation*}
This is sufficient because $G[0][\vect{n}] \beq F_0[\vect{n}] \bered [f_0(\vect{n})]$ so $G[0][\vect{n}] \bered [f_0(\vect{n})]$ since $[f_0(\vect{n})]$ is in normal form.
It is even enough to show that 
\begin{equation*}
    G[0] \beq F_0 \text{\quad and \quad} G[n + 1] \beq F_s[n](G[n])
\end{equation*}
...
and 
\begin{equation*}
    G[n + 1] \bered [n + 1]T([0], F_0)K_*  \beq ([n + 1], F_s [n](G[n]))K* \bered F_s[n](G[n])
\end{equation*}
...
We define 
\begin{equation*}
    W \coloneq Y (\lambda vy. Dy (v (\suc{y}))(Ty))
\end{equation*}
Then we get 
\begin{equation*}
    W[n] \bered D[n](W[n + 1])(T[n]) \bered 
    \begin{cases}
        [n] & \text{if } T[n] \bered [0] \\
        W[n + 1] & \text{if } T[n] \bered [m + 1] \text{ for some } m \in \bN
    \end{cases}
\end{equation*}
...
Now we set 
\begin{equation*}
    W' \coloneq \lambda \vect{x}. Y(\lambda vy. Dy(v(\suc{y})) (Ty\vect{x}))
\end{equation*}
and 
\begin{equation*}
    F \coloneq \lambda \vect{x}. U (W \vect{x} [0])
\end{equation*}
If $f(\vect{x})\down$ then 
\begin{equation*}
    F[\vect{n}] \bered U(W'[n][0]) \bered U([\mu_t(\vect{n})]) \bered [f(\vect{n})]
\end{equation*}
and if $f(\vect{x})\up$ then 
\begin{equation*}
    F[\vect{x}] \bered U(W'[0]) \bered U(W'[1]) \bered \dots
\end{equation*}
