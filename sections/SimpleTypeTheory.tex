...
\begin{boxdefi}\label{def:provejudge}
    ...
    \begin{enumerate}
        ...
        \item {(Functional extensionality)
            ...
            \AxiomC{}
            \UnaryInfC{$\Delta \vdash \forall f, g : \sigma \to \tau. (\forall x : \sigma. f x \mathrel{=_\tau} g x) \Rightarrow f \mathrel{=_{\sigma \to \tau}}$}
            \DisplayProof}
        \item {(Infinity) 
            
            \AxiomC{}
            \UnaryInfC{$\Delta \vdash \exists f : I \to I. (\forall x, x' : I. f x \mathrel{=_I} f x' \Rightarrow x = x') \wedge \exists y : I. \forall x : I. \neg (f x \mathrel{=_I} y)$}
            \DisplayProof}
...
\begin{rem}
    ...
    $\varepsilon_\tau P$ for $P : \tau \to \proptype$ is an arbitrary choice of a $t : \tau$ such that $P t$ holds if such that $t$ exists. 
...
\begin{rem}
    ...
    \begin{enumerate}
        \item {All types are non-empty.
            This is shown by applying the choice operator to $\varepsilon_\tau(\lambda x : \tau. \bot)$ which has type $\tau$ in any context.}
...
\begin{rem}
    If you remove $\varepsilon$ from Definition \ref{def:provejudge}, you get a constructive version of type theory.
\end{rem}
